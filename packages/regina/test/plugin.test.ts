...
test('plugin - uses prepareApplication from configured npm factory', async t => {
  const root = await mkdtemp(join(tmpdir(), 'regina-plugin-test-'))
  t.after(() => rm(root, { recursive: true, force: true }))
...
You are a test agent.`
  ...
  await mkdir(join(root, 'node_modules', 'regina-factory'), { recursive: true })
  await writeFile(
    join(root, 'node_modules', 'regina-factory', 'package.json'),
    JSON.stringify({ name: 'regina-factory', main: 'index.cjs' })
  )
  await writeFile(
    join(root, 'node_modules', 'regina-factory', 'index.cjs'),
    `exports.prepareApplication = async function prepareApplication (instanceId, definition) {
  return {
    id: instanceId,
    path: '/tmp/npm-factory-app',
    config: definition.id + ':' + instanceId,
    env: { FACTORY: 'npm' }
  }
}
`
  ...
  const addedApplication = management.added[0].apps[0] as any
  strictEqual(addedApplication.path, '/tmp/npm-factory-app')
  strictEqual(addedApplication.config, `test-agent:${instance.instanceId}`)
  strictEqual(addedApplication.env.FACTORY, 'npm')
})
